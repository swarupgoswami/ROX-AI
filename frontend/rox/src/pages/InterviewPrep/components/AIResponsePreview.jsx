        ...
        <div className="text-[14px] prose-slate dark:prose-invert max-w-none">
            <ReactMarkdown
              ...
              components={{
                ...
                td({children}){
                    return <td className='px-3 py-2 whitespace-nowrap txet-sm'>{children}</td>
                },
                hr(){
...
function codeBlock({code,language}){
    ...
    const copyCode=()=>{
        naviagator.clipboard.writeText(code);
        setCopied(true);
        setTimeout(()=>setCopied(false),2000);
    };

    return <div className="relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-20">
        <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200 ">
            <div className="flex items-center space-x-2">
                <LuCode size={16} className='text-xs font-semibold text-gray-600 uppercase tracking-wide'/>
                <span className=''>
                    {language || 'code'}
                </span>
            </div>
